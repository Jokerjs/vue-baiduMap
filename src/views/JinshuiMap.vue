...
import { decode } from '@/utils/parseGeoJson'
export default {
  name: 'JinshuiMap',
  data () {
    return {
  ...
  methods: {
    ...
    CustomData () {
      return this.axios.get('/static/city/410100.json').then((res) => {
        const resData = res.data
        const response = decode(resData)
        const features = response.features.splice(5, 1)
        // const response = res.data
        console.log(features)
        const nameArr = [].concat.apply([], features.map((names) => {
          return names.properties.name
        }))
        const RenderItemArr = features.map((coordinates) => {
          const coordinate = coordinates.geometry.coordinates
          return this.flattened(this.flattened(coordinate))
        })
